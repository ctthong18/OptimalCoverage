...
class LamdaWeight(nn.Module):
    ...
    def forward(self, states, actions):
        ...
        for curr_head_key, curr_head_agents, curr_head_action in zip(all_head_key, all_head_agents, all_head_action):
            x_key = torch.abs(curr_head_key).repeat(1, self.n_agents) + 1e-10 # (n_agents, )
            x_agents = torch.abs(curr_head_agents) # (n_agents, )
            x_action = torch.abs(curr_head_action) # (n_agents, )
            agent_action_weights = (x_agents * x_action)
            weights = x_key * agent_action_weights # (n_agents, )
        ...
        head_attend = head_attend/ math.sqrt(self.n_agents)
        head_attend = F.softmax(head_attend, dim=-1)
        return head_attend
