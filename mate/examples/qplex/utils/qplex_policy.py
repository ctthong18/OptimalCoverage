...
# from examples.qplex.utils.default_config import DEFAULT_CONFIG
...
class QPLEXLoss(nn.Module):
    ...
    def forward(
        ...
        if self.mixer is not None:
            ans_chosen, q_attend_regs, head_entropies = self.mixer(chosen_action_qvals, state, is_v = True)
            actions_onehot = F.one_hot(actions, num_classes = self.n_actions)
            ans_adv, _, _ = self.mixer(chosen_action_qvals, state, actions_onehot, max_action_vals = max_action_vals, is_v = False)
            chosen_action_qvals = ans_chosen + ans_adv

            target_chosen, _, _ = self.target_mixer(target_max_qvals, next_state, is_v = True)
            cur_max_actions_onehot = F.one_hot(cur_max_actions, num_classes = self.n_actions)
            target_adv, _, _ = self.target_mixer(target_max_qvals, next_state, actions=cur_max_actions_onehot, is_v = False)
...
def adjust_args(args):
    ...
    for k,v in additional_arguments.item():
...
class QPLEXTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        self.mixer = DuelMixer(config, self.n_agents, self.n_actions, self.env_global_state_shape, config['']).to(self.device)
        self.target_mixer = DuelMixer().to(self.device)
